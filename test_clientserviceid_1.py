...
# ClientServiceId를 1로 고정한 테스트 데이터 - Doctor 이벤트 타입 사용
test_data = {
    "ClientId": 83,  # Mrs Phoebe Smith의 OriginalPersonId
    "ClientServiceId": 1,  # 항상 1로 고정
    "EventDate": "2024-12-23T09:00:00.000Z",
    "ProgressNoteEventType": {
        "Id": 3  # Doctor 이벤트 타입 (eventtype.json에서 확인)
    },
    "NotesPlainText": "ClientServiceId = 1 고정값 테스트 - Doctor 이벤트 - Mrs Phoebe Smith",
    "CreatedByUser": {
        "FirstName": "Test",
        "LastName": "User",
        "UserName": "testuser",
...
def test_api():
    ...
    print("ClientServiceId = 1 고정값으로 Progress Note API 테스트 중...")
    print(f"URL: {url}")
    print(f"ClientId: {test_data['ClientId']}")
    print(f"ClientServiceId: {test_data['ClientServiceId']} (고정값)")
    ...
    try:
        ...
        if response.status_code == 200:
            print("\n✅ SUCCESS: ClientServiceId = 1로 API 호출 성공!")
        else:
